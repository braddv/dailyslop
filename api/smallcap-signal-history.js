...
async function ensureSchema() {
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS smallcap_signal_snapshots (
      snapshot_at TIMESTAMPTZ NOT NULL REFERENCES smallcap_signal_runs(snapshot_at) ON DELETE CASCADE,
  ...
  `;
  await sql`
    ALTER TABLE smallcap_signal_snapshots
    ADD COLUMN IF NOT EXISTS negative_long DOUBLE PRECISION NOT NULL DEFAULT 0
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS smallcap_signal_snapshots_symbol_time_idx
    ON smallcap_signal_snapshots (symbol, snapshot_at DESC)
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS smallcap_signal_snapshots_sector_time_idx
    ON smallcap_signal_snapshots (sector, snapshot_at DESC)
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS smallcap_signal_outcomes (
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS smallcap_signal_outcomes_type_time_idx
    ON smallcap_signal_outcomes (signal_type, snapshot_at DESC)
  `;
  await sql`
    ALTER TABLE smallcap_signal_outcomes
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS smallcap_signal_outcome_state (
  ...
  `;
  await sql`
    ALTER TABLE smallcap_signal_outcome_state
    ADD COLUMN IF NOT EXISTS materialization_version INTEGER NOT NULL DEFAULT 0
  `;
  await sql`
    DELETE FROM smallcap_signal_runs r
    WHERE NOT EXISTS (
      SELECT 1 FROM smallcap_signal_snapshots s WHERE s.snapshot_at = r.snapshot_at
    )
  `;
}
...
async function refreshSignalOutcomes(force = false) {
  ...
  await sql`
    INSERT INTO smallcap_signal_outcomes (
    ...
    WITH ordered AS (
      ...
      FROM smallcap_signal_snapshots s
    ),
    events AS (
      SELECT
        o.*,
  ...
  `;
  await sql`
    INSERT INTO smallcap_signal_outcome_state (
...
async function persistSnapshot(rows, sourceAsOf, kind) {
  ...
  await sql`
    INSERT INTO smallcap_signal_runs (snapshot_at, source_as_of, run_kind)
  ...
  await sql`
    INSERT INTO smallcap_signal_snapshots (
...
async function fetchMarketPayload(req, refresh) {
  const response = await fetch(`${requestOrigin(req)}/api/smallcaps${refresh ? "?refresh=true" : ""}`, {
